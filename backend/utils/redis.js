...
const thiryMonthOnSecond = 2592000;
module.exports = {
  ...
  setLongToken: async (token, user) => {
    return await redis.setex(token, thiryMonthOnSecond, JSON.stringify(user));
  },
  getLongToken: async token => {
    if (!token) return null;
    let sUser = await redis.get(token);
    if (!sUser) return null;
    if (sUser) {
      await redis.setex(token, thiryMonthOnSecond, sUser);
    }
